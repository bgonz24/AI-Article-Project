...
This is a program that gets the top trending science articles from NewsAPI, and based on the contents of these articles, finds similar articles among the J-Stories database to recommend to the user.
...
You will also need a service account key .json file to access the J-stories database on Firestore.
(I have included the API keys and access files I used in development, so there's no need to add anything)

The python files require a file called article_list.txt, which contains the URL slugs for all J-stories articles.  You can create this file by hand or use a script to generate it, but this is a required file.
...
- Repeats for all J-stories articles
